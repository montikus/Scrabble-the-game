...
export async function PUT(request, { params }) {
  try {
    ...
    logMessage.info(`Move ${id} has been updated`);
    return new Response(JSON.stringify(updatedMove), { status: 200 });
  } catch (error) {
    logMessage.error('Error updating move:', error);
...
export async function DELETE(request, { params }) {
  try {
    ...
    logMessage.info(`Move ${id} has been deleted`);
    return new Response(JSON.stringify({ message: 'Move has been deleted' }), { status: 200 });
  } catch (error) {
    logMessage.error('Can\'t delete move:', error);
