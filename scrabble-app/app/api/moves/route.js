...
export async function GET() {
  try {
    ...
    logMessage.error('Error getting moves:', error);
...
export async function POST(request) {
  try {
    ...
    logMessage.info('New move has been created and sent with MQTT:', newMove._id);

    return new Response(JSON.stringify(newMove), { status: 201 });
  } catch (error) {
    logMessage.error('Error creating move:', error);
